...
def _extrair_campos_pic(bloco: str) -> list:
    ...
    for ln in bloco.split('\n'):
        ...
        if re.search(r'(?i)\bREDEFINES\b', ln):
            continue
        m = re.match(r'^\s*(\d\d)\s+([A-Za-z][\w-]*)\s+.*?PIC(?:TURE)?\s+(?:IS\s+)?([0-9XASVP(),.\-]+)',
                     ln, re.IGNORECASE)
...
def _achar_pic_campo(nome_campo: str, content: str, copybooks: dict) -> tuple:
    """Acha a clausula PIC de um campo (nivel 2 digitos + nome + PIC),
    ignorando REDEFINES (aliases). Procura primeiro no proprio fonte, depois
    em cada copybook (nome_copy -> conteudo) na ordem dada. Retorna
    (pic, origem) ou (None, None) se nao encontrado em lugar nenhum."""
    padrao = re.compile(
        r'(?im)^\s*\d\d\s+' + re.escape(nome_campo) +
        r'\s+(?!.*\bREDEFINES\b).*?PIC(?:TURE)?\s+(?:IS\s+)?([0-9XASVP(),.\-]+)')
    m = padrao.search(content)
    if m:
        return m.group(1), None
    for nome_copy, texto_copy in copybooks.items():
        m = padrao.search(texto_copy)
        if m:
            return m.group(1), nome_copy
    return None, None


def estrutura_tabela(nome_convertido: str, tabela: str) -> dict:
    """Reconstroi a estrutura (coluna DB2 -> campo host-variable -> PIC) de
    uma tabela referenciada pelo fonte, cruzando 3 coisas que ja' existem no
    proprio codigo (nao e' inferencia - e' o que o fonte de fato declara):

    1. 'EXEC SQL DECLARE cursor CURSOR FOR SELECT col1, col2, ... FROM
       tabela ...' - lista de colunas, na ordem, por cursor.
    2. 'FETCH cursor INTO :hostvar1, :hostvar2, ...' do MESMO cursor - lista
       de host variables na MESMA ordem (posicao a posicao corresponde a
       coluna do SELECT).
    3. a declaracao '01/05/10 hostvar PIC ...' desse host variable, no
       proprio fonte ou em algum copybook referenciado (COPY nome.).

    Uma tabela pode ser lida por varios cursores (com colunas iguais ou
    diferentes) - as colunas de todos os cursores encontrados sao unidas
    (sem duplicar), mantendo a 1a definicao de host var/PIC encontrada.
    """
    source = CONVERTIDOS_DIR / nome_convertido
    if not source.exists():
        return {"tabela": tabela, "colunas": [], "cursores": []}
    content = source.read_text(encoding='latin-1', errors='ignore')

    alvo = tabela.strip().upper()
    # Cada bloco DECLARE e' isolado PRIMEIRO (ate' o proprio END-EXEC, sem
    # travar num nome de tabela especifico) e SO' DEPOIS filtrado pela
    # tabela procurada. Fazer a tabela parte do proprio regex de busca (como
    # numa 1a versao desta funcao) e' um erro sutil: quando o cursor sob
    # analise e' de OUTRA tabela, o '.*?' nao-guloso antes do nome da tabela
    # alvo nao encontra nada dentro do proprio bloco e continua expandindo
    # PARA ALEM do seu END-EXEC, atropelando o(s) proximo(s) DECLARE(s) ate'
    # achar a tabela certa em outro cursor completamente diferente - a
    # lista de colunas sai misturada com texto de blocos vizinhos.
    decl_re = re.compile(
        r'(?is)EXEC\s+SQL\s+DECLARE\s+([\w-]+)\s+CURSOR\s+FOR\s+SELECT\s+'
        r'(.*?)FROM\s+([\w]+\.[\w]+)(.*?)END-EXEC')

    cursores = []
    colunas_vistas = {}  # nome coluna -> dict (preserva 1a definicao)
    copybooks_cache: dict = {}

    for m in decl_re.finditer(content.upper()):
        cursor, colunas_txt, tabela_bloco = m.group(1), m.group(2), m.group(3)
        if tabela_bloco != alvo:
            continue
        colunas = [c.strip() for c in re.sub(r'\s+', ' ', colunas_txt).split(',') if c.strip()]

        fetch_re = re.compile(
            r'(?is)FETCH\s+' + re.escape(cursor) + r'\s+INTO\s+(.*?)END-EXEC')
        fm = fetch_re.search(content.upper())
        hostvars = []
        if fm:
            vars_txt = re.sub(r'\s+', ' ', fm.group(1))
            hostvars = [v.strip().lstrip(':').strip() for v in vars_txt.split(',') if v.strip()]

        cursores.append({"cursor": cursor, "colunas": len(colunas)})

        if not copybooks_cache:
            nomes_copy = sorted(set(mm.group(1).strip().upper()
                                     for mm in re.finditer(r'(?im)^\s*COPY\s+([A-Za-z0-9\-]+)', content)))
            # INFER_<programa>.cpy por ultimo: e' o copybook auto-gerado pra
            # host variables cujo copybook real (com o -X, os campos "crus"
            # do FETCH) nao foi entregue - so' e' injetado no fonte no
            # momento da compilacao (nao aparece como 'COPY' aqui no fonte
            # original), mas e' onde o PIC de fato mora quando nenhum dos
            # copybooks entregues declara o campo.
            nomes_copy.append(f'INFER_{nome_convertido}'.upper())
            for nome_cp in nomes_copy:
                cpy = COPY_DIR / f"{nome_cp}.cpy"
                if not cpy.exists():
                    cpy = next((f for f in COPY_DIR.glob('*.cpy') if f.stem.upper() == nome_cp), None)
                if cpy and cpy.exists():
                    try:
                        copybooks_cache[nome_cp] = cpy.read_text(encoding='latin-1', errors='ignore')
                    except Exception:
                        pass
        ...
        for i, coluna in enumerate(colunas):
            if coluna in colunas_vistas:
                continue
            campo = hostvars[i] if i < len(hostvars) else None
            pic, origem = (_achar_pic_campo(campo, content, copybooks_cache)
                           if campo else (None, None))
            colunas_vistas[coluna] = {
                "coluna": coluna, "campo": campo, "pic": pic,
                "origem": origem, "cursor": cursor,
            }

    return {
        "tabela": tabela.strip().upper(),
        "colunas": list(colunas_vistas.values()),
        "cursores": cursores,
    }
